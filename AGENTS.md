...
24 小时 worker 因为有需求分析指南，所以也可以自动化参与需求分析；但它参与的是
`analysis worker` 阶段：收集资料、精读原始需求、整理历史 session、找对标项目、
反向梳理半成品系统、生成业务对象/数据模型/状态流草案、列争议点和评审问题。
analysis worker 产出的是评审材料，不是定案。
