...
1. `debate_next`：需求尚未定案时的下一步。只能做需求分析、资料/竞品/参考项目调研、
   现状反向建模、业务场景追问、数据模型草案、状态流草案、架构方案比较、验收口径
   讨论、推翻旧方案、决策记录和评审材料整理。
2. `execution_next`：需求已经定案后的下一步。才能进入 24 小时 worker 实现、测试、
   提交、审计和修复。

定案前所有 next 都是博弈，不是实现任务。产品、业务、架构、测试、监控角色必须先
把目标、边界、数据模型、状态流、异常流、参考机制、验收标准和不做项对齐。哪怕是
“抄成熟项目”，也必须先做机制拆解和多轮评估；成熟项目是候选答案，不是直接结论。

产品角色不是文档整理员，而是产品质量压榨者：查资料、看市场/竞品/参考项目、追问
真实业务场景、推翻平庸方案、要求业务补充极端使用场景、要求架构给出更高效率和
稳定性方案，最后综合形成产品定案。业务角色确认真实规则和场景；架构角色确认数据、
状态、系统边界、性能和异常治理；测试角色把业务关注点变成可验证验收。

24 小时自动化不是只有执行阶段。完整主流程分两阶段：

1. `analysis/debate stage`：围绕需求持续博弈、查资料、找对标、反向建模、推翻方案、
   重订数据/状态/异常/验收，必要时可以跑很多轮，直到方案足够清楚。
2. `execution stage`：方案定好后，把已定案内容拆成大量小 `execution_next`，交给
   24 小时 worker 连续实现、测试、记录、修复。

这两阶段都是 24 小时自动化工程的一部分。需求分析博弈不是人工审批流，也不是临时
聊天总结；它本身应该被自动化成一个持续 debate/review/decision pipeline。当前这套
自动化还没完全做好时，可以先由当前交互页面临时代跑：人类/主控给方向，Codex 负责
组织博弈、监控 worker、发现偏差主动介入。最终形态必须是 24 小时机器自己先完成
需求博弈与方案定案，再生成大量执行 next。

当需求和方案已经非常明确时，后续应当极少人工介入。监控者的职责不是逐条审批所有
任务，而是监管主线、质量、执行链路、session/记忆、异常和漂移；发现偏差再主动
介入，纠偏后继续跑。

`execution worker` 只执行定案后的切片。任务包没有说明“本任务服务哪个主线、需求是否
定案、数据/状态合同是什么、参考机制是什么、验收点是什么、允许读改跑什么”时，
execution worker 不得自己补业务脑洞；必须返回需求未定案、需要建模或需要评审的
change request。
...
discussion：业务目标、真实问题、为什么现在做。
research：参考项目、市场/竞品、成熟机制、现有系统反向梳理。
modeling：业务对象、数据模型、字段含义、状态机、流程、异常、审计。
review：产品/业务/架构/测试/MOE 多角色博弈和推翻。
decision：记录定案、取舍、证据、过期支线、不做项。
execution：生成 backlog，交给 24 小时 worker 小切片执行。
```

执行和评审方法统一以 `docs/method.md` 为准。没有 closure，就只是讨论、
研究或分析产物，不能当成执行许可。
...
历史 plan-file 参考扫描已删除；当前任务只按 `docs/project.md` 和任务包
点名的 bounded slice 读取。
...
`docs/reference.md` 为准：现在先做 A9 自己的最小 plan lane
...
- MemPalace-first session/memory/context 主线：Codex raw JSONL ->
  `scripts/a9_codex_session_adapter.py` -> per-message drawer/evidence/index。
- session 人类快照、滚动总结和因果变迁：`docs/session.md`
- 项目当前状态：`docs/project.md`

旧的 session close-reading 和 session summary 文档已删除；如果聊天记忆和
MemPalace/evidence/raw JSONL 冲突，以 raw evidence 为准。

Session 分两类：

- 外部 Codex/operator session：人类和 Codex 窗口产生的原始 JSONL，用于
  MemPalace-compatible per-message drawer 初始化、恢复意图、提取 doctrine 和任务。
...
MemPalace recall 或 session 快照不会自动变成所有角色的知识。任何 product、architecture、test、
...
packet 明确注入的内容。把 recall 或 snapshot 写进上下文只是证据索引，不等于
角色已经理解。给 worker 派任务前，监控者必须把本次任务需要的因果记忆、过期
支线、当前边界和验收标准裁剪进任务包。新主控窗口必须先按当前 `AGENTS.md`、
`docs/project.md` 和 `docs/session.md`
接手。

Mobile/control plane 必须同时覆盖这两类 session。它不是只给 worker 做审批入口，
也不是只看 queue/runs；它必须把当前人类和主监控 Codex 的交互窗口也接进去：
看 operator chat tail、raw session index、MemPalace drawer/status、最近决策，并能从
手机触发 session adapter、legacy `session_refresh`/`session_close_reading` fallback、
compact、handoff 和下一任务。
...
- 24 小时 worker 默认不得执行下面的“Codex 自用流程”。只有
  ...
  external operator session 时，才允许读 `docs/session.md` 和 raw session。
...
文档过多本身就是风险。worker 不得把 `docs/`、归档目录、raw session、运行日志、
研究草稿和旧任务包当成同级事实源。
...
2. `docs/project.md`
3. `docs/method.md`
4. 当前任务 prompt / plan / decision packet

其他文档必须由任务包点名路径、用途和 bounded slice。`docs/session.md`、
`docs/mistakes.md`、`archive/original-ideas/*`
默认都是证据或原始材料，不是当前执行合同。

重订需求分析后必须做噪音处理：标记过期支线、归档旧任务、删除无事实价值的 scratch
文件、把仍有价值的 raw evidence 变成索引，并更新 `docs/session.md`
说明因果变迁。清理上下文是需求分析的一部分，不是后期美化。

代码噪音同理处理。未被当前主线、测试、运行证据或文档引用的旧代码路径，应进入
observation/repair；确认无事实价值后再删除。早期不要用固定行数或 token 数字硬卡，
但必须观测 broad read、过期入口、无主线文件和无证据功能。
...
`docs/session.md` 或 raw session JSONL。

开始继续任务前，先做三件事：

1. `python3 scripts/a9_service.py ps`，确认没有遗留后台 worker。
2. 读 `docs/session.md` 尾部，确认当前主线、过期支线和下一步 gate。
3. 必要时用 `scripts/a9_codex_session_adapter.py` 从 raw session 生成 bounded
   drawer evidence；legacy close-reading 只作 fallback/human snapshot。
4. 如果当前聊天上下文混乱，先用外部 session mini-flow 更新证据，再继续代码任务。

外部 Codex/operator session 的新默认 mini-flow：

```bash
python3 scripts/a9_codex_session_adapter.py convert \
  /root/.codex/sessions/2026/05/21/rollout-2026-05-21T11-20-49-019e488c-d5f9-7501-835a-bf6e8ff6d8a2.jsonl \
  --out .a9/mempalace/operator-session-drawers.jsonl
```

legacy fallback mini-flow：
...
legacy fallback flow 只做外部 session 治理：

- `session_refresh` 抽取 bounded JSON evidence。
- `session_close_reading` 追加 bounded 人类快照。
- 快照后必须更新 `docs/session.md`，把因果变迁、过期支线和当前决策统筹进去。
- 到当前尾部自动停。
...
- 已完成：MoE/monitor 从浅层 score 升级成轻量需求评审委员会，包含 product mainline、external learning、
...
- 已完成：OpenClaw policy attestation 第一版，summary/evidence/state/Redis session
  ...
  短引用。
- 下一刀：先暂停继续堆通讯功能，做外部 Codex/operator session 增量精读、因果链统筹、想法迭代细节、
