...
- 当前优先：把 MoE/monitor 从浅层 score 改成 `docs/moe-review-methodology.md`
  定义的轻量需求评审委员会。现有 `a9_monitor.py` observer 只是 prototype。
...
- 已完成：OpenClaw policy attestation 第一版，summary/evidence/state/Redis session
  ...
  短引用。
- 下一刀：先重构 monitor/MoE gate，再恢复 queued communication handler test 和
  strict task 长跑。不要跳过因果变迁统筹直接跑旧队列。
