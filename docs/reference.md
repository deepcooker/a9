...
| Headroom | Downloaded and source-run in A9 isolated environment. `headroom._core` build/import passed with Rust 1.95.0 under `.a9/rustup`; CCR tests passed `88/88`; content-router/decision/policy/cache-aligner/Codex/OpenClaw wrapper subset passed `225/227` with the two failures caused by Python 3.10 missing stdlib `tomllib`; proxy smoke passed `/livez`, `/readyz`, `/stats`, `/metrics`; Rust `headroom-core` full test hit Ubuntu 22.04 glibc vs ONNX Runtime `__isoc23_*` link incompatibility. | Compression, token savings, Codex/Claude/OpenAI proxy behavior, model-aware context reduction and observability. | Adopt mechanisms, not environment wholesale. Copy CCR protocol, retrieval hash, live-zone/cache-stability invariants, content routing, wrapper config hygiene and metrics. Keep heavy ML/ONNX compression optional/sidecar until A9 chooses a compatible runtime image. |
