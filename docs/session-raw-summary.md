...
A9 is currently the execution/control foundation:

```text
24h worker + monitor runtime
-> stable communication/control plane
-> evidence, trace, wrongbook and task context production
-> future private Agent OS
-> later financial/quant Codex and A3B training data loops
```

Do not collapse these layers:

- A9 runtime is the current engineering focus.
- A3B/A?B is the lower-level meta-cognitive activation system.
- Financial/quant model work comes after the runtime and evidence pipeline are
  stable.

## Current Decisions

- Requirements/debate precedes execution.
- Data model and state shape come before performance and gates.
- Gates observe first; only safety/data/license/destructive violations hard
  block early.
