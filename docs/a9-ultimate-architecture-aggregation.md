...
5. Barter-rs 应作为交易/通信/做市哪一层的主参考？哪些不能直接抄？
6. GPU/算力层应该选 Kubernetes + NVIDIA GPU Operator + KAI/Run:ai lineage，还是先用单机 Docker/systemd + vLLM？
7. 200GB+ 训练/推理镜像和大权重如何做秒级/近秒级启动？需要哪些缓存、预热、镜像和节点调度机制？
8. 需求方法论如何落入每个 agent/worker prompt，而不是只存在文档里？
9. MoE 评审角色是否应按 Product/Business/Architecture/Test/Security/Ops/Cost/Data 建模？
10. 当前 A9 已实现的 24h runtime 哪些保留、哪些重构、哪些删除？
11. 手机端是否应明确抄 GPT mobile + Codex CLI 交互模型，而不是后台 dashboard？
12. 24h worker 的 monitor view 应该暴露哪些意图、prompt、session、执行链和介入证据？
13. 手机端交易工作台应该先做哪些真实功能：行情/订单/持仓/资产/风控/告警/做市监控？
14. 算力币/DePIN/RWA 这条商业飞轮是否进入长期候选？如果进入，法律/合规/资产审计前置条件是什么？
15. 下一刀应该是继续通信治理、算力调度底座、手机主控入口，还是先统一文档和任务合同？
